...
for (const [, points] of radioTowers) {
    ...
    for (let firstTower = 0; firstTower < towerCount; firstTower++) {
        for (let secondTower = firstTower + 1; secondTower < towerCount; secondTower++) {
            ...
            // Towers are anti-nodes also
            antiNodes.add(`${pointA[0]},${pointA[1]}`);
            antiNodes.add(`${pointB[0]},${pointB[1]}`);
            ...
            let antiPointA = [pointB[0] + rowDifference, pointB[1] + colDifference];
            let antiPointB = [pointA[0] - rowDifference, pointA[1] - colDifference];

            // Make key out of antinodes
            let antiPointAKey = `${antiPointA[0]},${antiPointA[1]}`;
            let antiPointBKey = `${antiPointB[0]},${antiPointB[1]}`;

            while (in_bounds(antiPointA[0], antiPointA[1])) {
                if (!antiNodes.has(antiPointAKey)) {
                    antiNodes.add(antiPointAKey);
                }
                antiPointA = [antiPointA[0] + rowDifference, antiPointA[1] + colDifference]
                antiPointAKey = `${antiPointA[0]},${antiPointA[1]}`;
            }

            while (in_bounds(antiPointB[0], antiPointB[1])) {
                if (!antiNodes.has(antiPointBKey)) {
                    antiNodes.add(antiPointBKey);
                }
                antiPointB = [antiPointB[0] - rowDifference, antiPointB[1] - colDifference]
                antiPointBKey = `${antiPointB[0]},${antiPointB[1]}`;
            }
